...
function renderTrend(doc){{const p=document.querySelector(`#trPanels .dp[data-key="${{doc}}"]`);if(CR['tr'+doc])return;const id='trc'+DOCS.indexOf(doc);p.innerHTML=`<div class="cw tall"><canvas id="${{id}}"></canvas></div>`;const dd=DAILY[doc];CR['tr'+doc]=new Chart(document.getElementById(id),{{type:'bar',data:{{labels:dd.map(r=>r.dt.slice(5)+'('+r.dy+')'),datasets:[{{label:'입원',data:dd.map(r=>r.i),backgroundColor:'rgba(78,154,241,.72)',borderRadius:3,stack:'s'}},{{label:'외래',data:dd.map(r=>r.o),backgroundColor:'rgba(244,162,45,.72)',borderRadius:3,stack:'s'}}]}},options:co({{plugins:{{legend:{{display:false}}}}}})}}); }}
renderTrend(DOCS[0]);
