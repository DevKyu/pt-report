...
def _edit_distance(a: str, b: str) -> int:
    ...
    if abs(len(a) - len(b)) > 1:
        return 99
    if len(a) > 10 or len(b) > 10:  # 긴 이름은 스킵
        return 99
    m, n = len(a), len(b)
    dp = list(range(n + 1))
...
def load_and_process(filepath: "str | Path") -> Dict[str, Any]:
    ...
    try:
        # xls(구버전)는 xlrd, xlsx는 openpyxl 자동 선택
        engine = 'xlrd' if fp.suffix.lower() == '.xls' else 'openpyxl'
    ...
    df['처방의']  = df['처방의'].apply(_normalize_doctor)  # 처방의명 정규화
    df['접수일자'] = pd.to_datetime(df['접수일자'], errors='coerce').dt.date
    ...
    # 처방의명 이슈 감지 (원본 df에서)
    raw_warnings = detect_doctor_name_issues(
        pd.read_excel(fp, sheet_name=0, usecols=['처방의']).dropna()
    )
    ...
    return {
        ...
        'filepath':   fp,
        'warnings':   warnings,  # 처리 중 감지된 경고 목록
    }
...
def load_and_merge(filepaths: list) -> Dict[str, Any]:
    ...
    # 처방의명 이슈 감지
    all_warnings += detect_doctor_name_issues(merged[['처방의']])
    ...
    # 이하 load_and_process와 동일한 집계 로직
    daily = (
        dedup.groupby(['처방의','접수일자','구분'])
        .size().unstack(fill_value=0).reset_index()
    )
    ...
    return {
        ...
        'period':     period,
        'year_month': year_month,
        'filepath':   Path(filepaths[-1]),  # 최신 파일 기준
